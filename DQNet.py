...
def playGame(train_indicator=1):  # 1 means Train, 0 means simply Run
    ...
    W = tf.Variable(tf.random_uniform([8, 3], 0, 0.01))
    ...
    for i in range(episode_count):
        ...
        print("Episode : " + str(i) + " Replay Buffer " + str('disabled'))
        ...
        for j in range(max_steps):
            ...
            a_t, allQ = sess.run([predict, Qout], feed_dict={inputs1: np.identity(8)[s_t:s_t + 1]})
            ...
            Q1 = sess.run(Qout, feed_dict={inputs1: np.identity(8)[s1:s1 + 1]})
            # Obtain maxQ' and set our target value for chosen action.
            maxQ1 = np.max(Q1)
            targetQ = allQ
            targetQ[0, a_t[0]] = r_t + GAMMA * maxQ1

            if (train_indicator):

                # Train our network using target and predicted Q values
                _, W1 = sess.run([updateModel, W], feed_dict={
                    inputs1: np.identity(8)[s:s + 1], nextQ: targetQ})
                # rAll += r_t
                # s = s1
